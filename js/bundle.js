(() => {
  ...
  var init_dom = __esm({
    "js/dom.js"() {
      init_state();
      isFirstRender = true;
      getSquare = (i, j) => document.querySelector(`.child[data-i="${i}"][data-j="${j}"]`);
      clearDots = () => {
  ...
  // js/logic.js
  var isUnderAttack, isMoveSafe, hasAnyValidMoves, checkDrawConditions;
  var init_logic = __esm({
    "js/logic.js"() {
      init_state();
      isUnderAttack = (r, c, color, board = state.board) => {
      ...
      isMoveSafe = (startI, startJ, targetI, targetJ, color = state.currentTurn) => {
      ...
      hasAnyValidMoves = (color = state.currentTurn) => {
        for (let i = 0; i < 8; i++) {
          for (let j = 0; j < 8; j++) {
            const piece = state.board[i][j];
  ...
  var init_ui = __esm({
    "js/ui.js"() {
      ...
      init_dom();
      init_logic();
      markMove = (targetI, targetJ, startI, startJ, isHover = false) => {
  ...
  // js/animations.js
  var playMoveAnimation, playCombatAnimation, showExplosion, createParticles, triggerShockwave, playCastlingRookAnimation, playPromotionAscension;
  var init_animations = __esm({
    "js/animations.js"() {
      init_dom();
      init_state();
      playMoveAnimation = (startI, startJ, targetI, targetJ, pieceType, isCapture, isEnPassant, actualVictimSq) => {
      ...
      playCombatAnimation = (attackerSq, targetSq, pieceImg, resolve, attackerType, isEnPassant) => {
      ...
      playCastlingRookAnimation = (startI, startJ, targetI, targetJ) => {
        return new Promise((resolve) => {
      ...
      playPromotionAscension = (targetI, targetJ, color, chosenType) => {
        return new Promise((resolve) => {
          const sq = getSquare(targetI, targetJ);
  ...
  var init_network = __esm({
    "js/network.js"() {
      ...
      emitMove = (startI, startJ, targetI, targetJ, promotionType = null) => {
        if (!socket || !roomCode) return;
        if (state.isExecutingNetworkMove) return;
        socket.emit("move", {
          roomCode,
  ...
  var init_game = __esm({
    "js/game.js"() {
      ...
      getAlgebraic = (piece, startI, startJ, targetI, targetJ, captured) => {
      ...
      addCapturedToPanel = (piece, victimI, victimJ) => {
        const img = document.createElement("img");
        img.src = `./asserts/${piece.color}${piece.type}.png`;
