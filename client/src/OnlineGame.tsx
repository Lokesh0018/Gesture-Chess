...
export default function OnlineGame() {
    ...
    newSocket.on('opponent_move', (move) => {
      setGame((g) => {
        ...
        try {
          ...
          if (result.captured) {
            audio.capture();
            vfx.triggerFromSquare('capture', result.to, orientation, boardElement, g.turn() === 'w' ? '#ebecd0' : '#739552');
          }
          if (result.promotion) {
            audio.promote();
  ...
  function makeAMove(move: any) {
    ...
    try {
      ...
      if (result.captured) {
        audio.capture();
        vfx.triggerFromSquare('capture', result.to, orientation, boardElement, game.turn() === 'w' ? '#ebecd0' : '#739552');
      }
      if (result.promotion) {
        audio.promote();
        setCinematic({ square: result.to, color: result.color as 'w'|'b', type: result.promotion as 'q'|'r'|'b'|'n' });
